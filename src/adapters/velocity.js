...
function extractFindings(report) {
  const list = Array.isArray(report?.issues)
    ? report.issues
...
export async function runVelocity(root) {
  try {
    ...
    const errors = (report.discoveryErrors ?? []).map(
    ...
    return engineResult({
      ...
      score: Number.isFinite(report.score) ? Number(report.score) : null,
    ...
    if (error?.code === "ERR_MODULE_NOT_FOUND") {
      return engineResult({
        engine: "performance",
        status: "unavailable",
        errors: ["@wess2001/velocity is not installed"],
      });
    }
    return engineResult({
      engine: "performance",
